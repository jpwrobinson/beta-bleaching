...
names(cols.reduced)<-c('bl', 'ba', 'bf', 'bg', 'bh', 'bi', 'bj', 'bk', 'a')
...
params.fg<-ggplot(post.fg, aes(x = dist, y = x.num, alpha=alpha, col=state)) + 
		...
		geom_halfeyeh(size=5, trim=TRUE,  fill=NA,alpha=0.8, .width=0.5) + 
		geom_halfeyeh(size=1, trim=TRUE, fill=NA, alpha=1, .width=0.95) + 
...
params.fix<-ggplot(post.fix, aes(x = dist, y = x.num, alpha=alpha)) + 
		geom_vline(xintercept=0, linetype='dashed', size=0.5, col='black') +
		geom_halfeyeh(size=1, fill=NA, alpha=0, .width=0.95) + 
		geom_halfeyeh(size=5, fill=NA, alpha=0, .width=0.5) + 
		labs(y = '', x ='Standardized effect size') +
...
## print ggs 
plot_grid(params.fg, params.fix, labels = c('(b)','(c)'), 
		nrow=1, hjust=0.02, label_size=12)
